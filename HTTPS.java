...
public class HTTPS extends NanoHTTPD {
    ...
    // ✅ SSL Keystore settings — update these paths/passwords as needed
    private static final String keystorePath = "C:\\Users\\Public\\Documents\\keystore.jks";
    private static final String keystorePassword = "password123";

    public HTTPS() throws IOException {
        super(8443); // HTTPS port (changed from 8080)

        // ✅ Enable SSL/TLS using the keystore
        try {
            makeSecure(
                NanoHTTPD.makeSSLSocketFactory(keystorePath, keystorePassword.toCharArray()),
                null
            );
            System.out.println("🔒 SSL enabled successfully!");
        } catch (Exception e) {
            System.err.println("⚠️  SSL setup failed, falling back to HTTP: " + e.getMessage());
        }
        ...
        System.out.println("🌐 Access via Hostname: https://" + hostName + ":8443/");
        System.out.println("💻 Access via IP:        https://" + hostAddress + ":8443/");
        System.out.println("ℹ️  First visit: Click 'Advanced' → 'Proceed' to accept the self-signed certificate.");
    }
    ...
    private Response serveScreenshots(String uri) {
        try {
    ...
    private String buildHtmlFromExcels() {
        ...
        html.append("<div style='position: relative; text-align: center;'>")
            ...
            .append("<button class='screenshot-btn' onclick=\"window.open('/screenshots','_blank')\">ScreenShot</button>")
            .append("</div>");
